...
// G2O Optimization
class VertexPose : public g2o::BaseVertex<6, Sophus::SE3d> // 6 refers to update vector's(vertex dimension) dimension
{   
    public:
...
class EdgeProjection : public g2o::BaseUnaryEdge<1, double, VertexPose> // 1 refers to resdual dimension. Since we only have pose  vertex, we use "BaseUnaryEdge"
{ // "2": residual vector dimension, "Vector2d":resdiual type, 3rd parameter is vertex class 
    public:
        EIGEN_MAKE_ALIGNED_OPERATOR_NEW;

        EdgeProjection(cv::Mat& img2, Eigen::Vector2d& pix_ref, Eigen::Matrix3d& K, double depth_ref):
                    img2_(img2), pix_ref_(pix_ref), K_(K), depth_ref_(depth_ref) { }
        ...
        {   
            ...
            if(status_) // BU HATALI OLABILIR !!!!
                _error(0,0) = _measurement - GetPixelValue(img2_, u, v); // _measurement is ref. img intensity(e.g. I_1)
        }
...
int main(int argc, char** argv)
{
    ...
    {
        cv::Point2f p(pixels_ref[i](0,0), pixels_ref[i](1,0));
        circle(left_img, p, 1, cv::Scalar(0,255,0), cv::FILLED, cv::LINE_8);
    }

    cv::imshow("points", left_img);
    cv::waitKey(0);
    */
...
void MotionOnlyBAG2O(vector<Eigen::Vector2d>& ref_points2d, cv::Mat& img1, cv::Mat& img2, vector<double>& ref_depth, Sophus::SE3d& pose)
{
    ...
    typedef g2o::BlockSolver<g2o::BlockSolverTraits<6, Eigen::Dynamic>> BlockSolverType; // 6 refers to dimension of vertices. 
    ...
    for(size_t i=0; i<ref_points2d.size(); i++)
    {
        ...
        auto pix_ref = ref_points2d[i];
        EdgeProjection* edge = new EdgeProjection(img2, pix_ref, K_eigen, depth);
        edge->setId(index);
        edge->setVertex(0, vertex_pose);
        float meas = GetPixelValue(img1, pix_ref[0], pix_ref[1]);
        edge->setMeasurement(meas); // meas: referans frame'deki pixelin intensity degerini ifade ediyor
